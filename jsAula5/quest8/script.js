...
pai.addEventListener("mousedown", function (e) {
	...
	if (e.target != pai) {
		...
		// rect.x
		//	// fornece a posição horizontal (eixo X) do canto superior esquerdo do elemento, em relação à janela de visualização (viewport).
		// rect.y
		//	// fornece a posição vertical (eixo Y) do canto superior esquerdo do elemento, em relação à janela de visualização (viewport).
		// rect.width
		// rect.height
	}
});
